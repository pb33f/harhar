...
func (c *Recorder) RoundTrip(req *http.Request) (*http.Response, error) {
    ...
    trace := &httptrace.ClientTrace{
        ...
        GotConn: func(connInfo httptrace.GotConnInfo) {
            ent.Timings.Blocked = float64(time.Since(connWaitStart).Milliseconds())
        },
        ...
        DNSDone: func(dnsInfo httptrace.DNSDoneInfo) {
            ent.Timings.DNS = float64(time.Since(dnsStart).Milliseconds())
        ...
        ConnectDone: func(network, addr string, err error) {
            ent.Timings.Connect = float64(time.Since(connStart).Milliseconds())
            sendStart = time.Now()
        },
        ...
        TLSHandshakeDone: func(connState tls.ConnectionState, err error) {
            ent.Timings.SSL = float64(time.Since(tlsStart).Milliseconds())
        },

        WroteRequest: func(info httptrace.WroteRequestInfo) {
            ent.Timings.Send = float64(time.Since(sendStart).Milliseconds())
            waitStart = time.Now()
        },
        GotFirstResponseByte: func() {
            ent.Timings.Wait = float64(time.Since(waitStart).Milliseconds())
            respStart = time.Now()
        },
    }
    ...
    ent.Timings.Receive = float64(time.Since(respStart).Milliseconds())
